<template>
  ...
  <div class="box">
      ...
      <vue-loading type="spiningDubbles" color="#d9544e" :size="{ width: '70px', height: '70px' }"></vue-loading> 
      </div>  
...
export default {
  ...
  methods:{
    ...
    async getfuArry () {
      ...
      that.$http.get('api/lottery-v1/xjssc/odds/').then(function(res){
    ...
    getarr () {
      let that = this;
      that.$http.get('/api/lottery-v1/xjssc/opentime/').then(function(res){
...
<style <style scoped>
.li_1 i{
  background: -webkit-gradient(linear, left top, left bottom, from(#F56C6C), to(#F0EDE7));
  background: -webkit-linear-gradient(#F56C6C, #F0EDE7);
  background: linear-gradient(#F56C6C, #F0EDE7);
}
#result{
  background: -webkit-gradient(linear, left top, left bottom, from(#F56C6C), to(#F0EDE7));
  background: -webkit-linear-gradient(#F56C6C, #F0EDE7);
