          ...
          <div class="lottery_w_box">
            <ul class="cq_ul2">
              <li v-for="(item,indexs) in ratetype" :class="item.selected?'choseBox':''" @click="selected(item,index)" v-if="forLength[index] == ratetype[indexs].playd_type_name" :key="item.id">
                <span class="luckyBox">
                <span class="li_1 " v-if="item.playd_type == 60151 && item.name<=20"><i>{{item.name}}</i></span>
                <span class="li_1 " v-else-if="item.playd_type == 60151 && item.name<=40"><a>{{item.name}}</a></span>
                <span class="li_1 " v-else-if="item.playd_type == 60151 && item.name<=60"><b><span>{{item.name}}</span></b></span>
                <span class="li_1 " v-else-if="item.playd_type == 60151 && item.name<=80"><tt>{{item.name}}</tt></span>
...
this.allmoney = NP.strip(sum);        }
    ...
    getfuArry () {
      ...
      that.$http.get('api/lottery-v1/bjkl8/odds/').then(function(res){
    ...
    getarr () {
      let that = this;
      that.$http.get('/api/lottery-v1/bjkl8/opentime/').then(function(res){
...
<style scoped>
.li_1 a{
  background: -webkit-gradient(linear, left top, left bottom, from(#00AAAA), to(#F0EDE7));
  background: -webkit-linear-gradient(#00AAAA, #F0EDE7);
  background: linear-gradient(#00AAAA, #F0EDE7);
...
.li_1 b{
  background: -webkit-gradient(linear, left top, left bottom, from(#FF4500), to(#F0EDE7));
  background: -webkit-linear-gradient(#FF4500, #F0EDE7);
  background: linear-gradient(#FF4500, #F0EDE7);
  display: inline-block;
  height: 26px;
  width: 26px;
  border-radius: 50%;
...
.li_1 tt{
  background: -webkit-gradient(linear, left top, left bottom, from(	#6A5ACD), to(#F0EDE7));
  background: -webkit-linear-gradient(#6A5ACD, #F0EDE7);
  background: linear-gradient(#6A5ACD, #F0EDE7);
  display: inline-block;
  height: 26px;
  width: 26px;
  border-radius: 50%;
