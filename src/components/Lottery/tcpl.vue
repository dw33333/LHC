...
export default {
  ...
  methods:{
    ...
    async getfuArry () {
      ...
      that.$http.get('api/lottery-v1/tcpl3/odds/').then(function(res){
    ...
    getarr () {
      let that = this;
      that.$http.get('/api/lottery-v1/tcpl3/opentime/').then(function(res){
...
<style scoped>
.li_1 i{
  background: -webkit-gradient(linear, left top, left bottom, from(#409EFF), to(#F0EDE7));
  background: -webkit-linear-gradient(#409EFF, #F0EDE7);
  background: linear-gradient(#409EFF, #F0EDE7);
}
#result{
  background: -webkit-gradient(linear, left top, left bottom, from(#409EFF), to(#F0EDE7));
  background: -webkit-linear-gradient(#409EFF, #F0EDE7);
