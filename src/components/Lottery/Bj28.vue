  ...
  <div class="box">
      <div class="content">
        ...
        <div class="table1" v-show="showtihuan">
          ...
          <div style="display: inline-block;margin-bottom:-3px;background:#fff;">
            <ul class="cq_ul2">
              <li v-for="(item,indexs) in ratetype" :class="item.selected?'choseBox':''" @click="selected(item,index)" v-if="forLength[index] == ratetype[indexs].playd_type_name" :key="item.id">
                <span  class="luckyBox">
                <span class="li_1 " v-if="item.playd_type == 60187"><i>{{item.name}}</i></span>
...
export default {
  ...
  methods:{
    ...
    getfuArry () {
      ...
      that.$http.get('api/lottery-v1/bj28/odds/').then(function(res){
    ...
    getarr () {
      let that = this;
      that.$http.get('/api/lottery-v1/bj28/opentime/').then(function(res){
