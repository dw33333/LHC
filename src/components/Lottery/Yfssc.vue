...
export default {
  ...
  methods:{
    ...
    async getfuArry () {
      ...
      that.$http.get('api/lottery-v1/Yfssc/odds/').then(function(res){
    ...
    getarr () {
      let that = this;
      that.$http.get('/api/lottery-v1/Yfssc/opentime/').then(function(res){
...
<style scoped>
.li_1 i{
  background: -webkit-gradient(linear, left top, left bottom, from(#FF0033), to(#F0EDE7));
  background: -webkit-linear-gradient(#FF0033, #F0EDE7);
  background: linear-gradient(#FF0033, #F0EDE7);
  color: #fff;
}
#result{
  background: -webkit-gradient(linear, left top, left bottom, from(#FF0033), to(#F0EDE7));
