  ...
  <div class="box">
      <div class="content">
        ...
        <div class="table1" v-show="showtihuan">
          <div v-for="(items,index) in forLength.length" :key="items.id" class="lottery_o_box">
          ...
          <div class="lottery_w_box">
            <ul class="cq_ul2">
              <li v-for="(item,indexs) in ratetype" :class="item.selected?'choseBox':''" @click="selected(item,index)" v-if="forLength[index] == ratetype[indexs].playd_type_name" :key="item.id">
                <span  class="luckyBox">
                <span class="li_1 " v-if="item.playd_guoup_id == 702" :class="'car'+item.name"></span>
...
export default {
  ...
  methods:{
    ...
    getfuArry () {
      ...
      that.$http.get('api/lottery-v1/bjpk10/odds/').then(function(res){
    ...
    getarr () {
      let that = this;
      that.$http.get('/api/lottery-v1/bjpk10/opentime/').then(function(res){
