...
export default {
  ...
  methods:{
    ...
    getfuArry () {
      ...
      that.$http.get('api/lottery-v1/hnklsf/odds/').then(function(res){
    ...
    getarr () {
      let that = this;
      that.$http.get('/api/lottery-v1/hnklsf/opentime/').then(function(res){
...
<style scoped>
.li_1 i{
  background: -webkit-gradient(linear, left top, left bottom, from(#330033), to(#F0EDE7));
  background: -webkit-linear-gradient(#330033, #F0EDE7);
  background: linear-gradient(#330033, #F0EDE7);
  color: #fff;
}
#result{
  background: -webkit-gradient(linear, left top, left bottom, from(#330033), to(#F0EDE7));
