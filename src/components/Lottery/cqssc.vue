...
export default {
  ...
  methods:{
    ...
    async getfuArry () {
      ...
      that.$http.get('api/lottery-v1/cqssc/odds/').then(function(res){
    ...
    getarr () {
      let that = this;
      that.$http.get('/api/lottery-v1/cqssc/opentime/').then(function(res){
...
<style <style scoped>
.li_1 i{
  background: -webkit-gradient(linear, left top, left bottom, from(#1D8348), to(#F0EDE7));
  background: -webkit-linear-gradient(#1D8348, #F0EDE7);
  background: linear-gradient(#1D8348, #F0EDE7);
}
#result{
  background: -webkit-gradient(linear, left top, left bottom, from(#1D8348), to(#F0EDE7));
  background: -webkit-linear-gradient(#1D8348, #F0EDE7);
