...
export default {
  ...
  methods:{
    ...
    getfuArry () {
      ...
      that.$http.get('api/lottery-v1/txffc/odds/').then(function(res){
    ...
    getarr () {
      let that = this;
      that.$http.get('/api/lottery-v1/txffc/opentime/').then(function(res){
...
<style scoped>
.li_1 i{
  background: -webkit-gradient(linear, left top, left bottom, from(#34495E), to(#99A3A4));
  background: -webkit-linear-gradient(#34495E, #99A3A4);
  background: linear-gradient(#34495E, #99A3A4);
  color: #fff;
}
#result{
  background: -webkit-gradient(linear, left top, left bottom, from(#34495E), to(#99A3A4));
