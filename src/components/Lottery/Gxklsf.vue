...
export default {
  ...
  methods:{
    ...
    getfuArry () {
      ...
      that.$http.get('api/lottery-v1/gxklsf/odds/').then(function(res){
    ...
    getarr () {
      let that = this;
      that.$http.get('/api/lottery-v1/gxklsf/opentime/').then(function(res){
...
<style scoped>
.li_1 i{
  background: -webkit-gradient(linear, left top, left bottom, from(#DF013A), to(#F0EDE7));
  background: -webkit-linear-gradient(#DF013A, #F0EDE7);
  background: linear-gradient(#DF013A, #F0EDE7);
  color: #fff;
}
#result{
  background: -webkit-gradient(linear, left top, left bottom, from(#DF013A), to(#F0EDE7));
