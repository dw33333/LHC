...
export default {
  ...
  methods:{
    ...
    async getfuArry () {
      ...
      that.$http.get('api/lottery-v1/Tjssc/odds/').then(function(res){
    ...
    getarr () {
      let that = this;
      that.$http.get('/api/lottery-v1/Tjssc/opentime/').then(function(res){
...
<style scoped>
.li_1 i{
  background: -webkit-gradient(linear, left top, left bottom, from(#303133), to(#F0EDE7));
  background: -webkit-linear-gradient(#303133, #F0EDE7);
  background: linear-gradient(#303133, #F0EDE7);
  color: #fff;
}
#result{
  background: -webkit-gradient(linear, left top, left bottom, from(#303133), to(#F0EDE7));
